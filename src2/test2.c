...
void draw(void *c)
{
    ...
    gkit_widget *p = (gkit_widget*)c;
    gkit_widget_canvas_draw_arc(p, 80, 70, 300, 100, 100, 360*64);
    gkit_widget_canvas_draw_line(p, 10, 10, 100, 100);
    /*gkit_widget_canvas_set_string_font_size(p, STK_FONT_SIZE_9x15);*/
    gkit_widget_canvas_draw_string(p, 50, 50, "50x50");

    for(i = 350; i > 200; i--)
        gkit_widget_canvas_draw_point(p, 100+i, i-150);        
}
...
void color(void *c)
{
    ...
    gkit_window_set_color((gkit_widget*)w->d1, w->c);
}
...
void move(void *c)
{
    ...
    stk_widget_set_pos((gkit_widget*)w->d1, w->x, w->y);
}
...
int main()
{
    ...
    win = gkit_window_new(500, 500, 800, 600, "STK Demo");
